...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName('set')
                ...
                .addStringOption(opt =>
                    opt.setName('time')
                        .setDescription('送信日時 (例: 2026-12-31 23:59)')
                        .setRequired(true)
                )
        )
        .addSubcommand(sub =>
    ...
    async execute(interaction, pool) {
        ...
        if (subcommand === 'set') {
            ...
            await interaction.editReply({ content: `✨ ${channel} へのメッセージ送信を **${targetDate.toLocaleString('ja-JP')}** に予約しました！` });
            ...
            for (const row of res.rows) {
                const dateStr = new Date(row.send_at).toLocaleString('ja-JP');
